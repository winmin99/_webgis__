...
const distPath = rootPath + '/dist' + '/public';
const srcPath = rootPath + '/src';
const configPath = rootPath + '/';
const assetDistPath = distPath + '/assets';
const assetSrcPath = rootPath + '/src/assets';

// page scripts and styles
const pageScripts = glob.sync(assetSrcPath + '/js/pages/**/!(_*).js');
const pagesScss = glob.sync(assetSrcPath + '/sass/pages/**/!(_*).scss');
...
(async () => {
  await del.sync(distPath, { force: true });
})();
...
// process skin scss
const skinScss = glob.sync(
  assetSrcPath + '/sass/**/skins/**/!(_*|style*).scss'
);
        ...
        rules: [
          {
            search: /inc\/api\//i,
            replace:
              'https://keenthemes.com/' + themeName + '/tools/preview/api/'
          }
        ]
      }
    ])
  ...
  return {
                ...
                if (param.tag === 'images') {
                  param.name.split(',').forEach((file) => {
                    if (file) {
                      ...
                      // copy image
                      fs.copyFileSync(
                        fs.realpathSync(assetSrcPath + '/' + file),
                        outputPath + path.basename(file)
                      );
                    }
                  });
                }
    ...
    module: {
      rules: [
        ...
        {
          test: /\.(gif|png|jpe?g)$/,
          include: [path.resolve(__dirname, 'node_modules'), rootPath],
          use: [
            {
              loader: 'file-loader',
              options: {
                ...
                outputPath: (url, resourcePath) => {
                  ...
                  if (matched) {
                    for (let i = 0; i < filesConfig.length; i++) {
                      if (
                        filesConfig[i].filename.match(new RegExp(matched[1]))
                      ) {
                        let output = '';
...
function pathWithoutFile(filename) {
