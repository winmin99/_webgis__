...
DROP VIEW IF EXISTS public.viw_wtl_flow_ps;

CREATE OR REPLACE VIEW public.viw_wtl_flow_ps
            (geom, 레이어, 관리번호, 급수구역, 구경, 유량계형식, 유량계종류, 제작회사명, 읍면동, 법정동, 급수분구, 설치일자, 공사번호, 관리기관, 도엽번호, 관로관리번호, 방향각) AS
SELECT flow_tb.geom,
       '유량계'::text     AS "레이어",
...
FROM wtl_flow_ps flow_tb
         ...
         LEFT JOIN wtl_wtssa_as wsb_tb ON flow_tb.wsb_cde = wsb_tb.wsb_cde
