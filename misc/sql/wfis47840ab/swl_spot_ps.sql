ALTER TABLE public.swl_spot_ps
    ADD COLUMN IF NOT EXISTS geom geometry(MultiPoint, 5187);

DROP VIEW IF EXISTS public.viw_swl_spot_ps;

CREATE OR REPLACE VIEW public.viw_swl_spot_ps
            (geom, 레이어, 관리번호, 처리구역, 물받이용도, 물받이형태, 내경, 가로길이, 세로길이, 깊이, 뚜껑형태, 뚜껑재질, 방향각, 읍면동, 법정동, 처리분구, 설치일자, 최종준설일자,
             공사번호,
             관리기관, 도엽번호)
AS
...
FROM swl_spot_ps ms
         ...
         LEFT JOIN private.cd_sbd smu ON ms.sbd_cde = smu.codeno
